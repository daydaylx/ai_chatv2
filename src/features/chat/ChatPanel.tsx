...
export default function ChatPanel() {
  ...
  const onSend = async () => {
    ...
    // Bestehende Messages => ChatMsg[], dann aktuellen User anhängen
    const baseHistory: ChatMsg[] = sess.messages.map((m) => ({
      role: m.role,
      content: m.content,
    }));
    const history: ChatMsg[] = [...baseHistory, { role: "user", content: text }];

    // Modell für Summary/Memory (fällt auf Hauptmodell zurück). Non-null nach obiger Guard.
    const runModel = (settings.summarizerModelId ?? settings.modelId)!;

    // (Optional) Systemprompt könnte hier injiziert werden (Persona)
    await streamChat({
      apiKey,
      model: settings.modelId!, // garantiert gesetzt durch Guard oben
      messages: history,
      ...
      onDone: async (full) => {
        ...
        // Auto-Summary (nur wenn Schwellwerte erreicht)
        try {
          if (settings.autoSummarize && shouldSummarize(history)) {
            ...
            // Unaufdringliche Kurzfassung als Assistant-Nachricht
            const bullets = summary.bullets.length ? `• ${summary.bullets.join("\n• ")}` : "";
            const narrative = summary.narrative ? `\n\n${summary.narrative}` : "";
            if (bullets || narrative) {
              await sess.appendAssistant(`(Zusammenfassung)\n${bullets}${narrative}`);
            }
          }
        } catch (e) {
      ...
      signal: ctrl.signal,
      maxTokensOut: 8000, // Hard-Limit gegen ausufernde Kosten
    });
  };
        ...
        {view.map((m) => (
          <div
            ...
            className={[
              ...
              m.role === "user"
                ? "ml-auto bg-[hsl(var(--accent-600))] text-black"
