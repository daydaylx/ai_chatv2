...
export type StyleInfo = { id: string; name: string; description?: string; system?: string; };

type Props = { styles: StyleInfo[]; selectedId?: string | null; onSelect: (id: string) => void; };
  ...
  return (
    <div role="listbox" tabIndex={0} onKeyDown={onKey} className="flex flex-col gap-2 outline-none">
      {styles.map((s, i) => {
        ...
        return (
          <button key={s.id} type="button" role="option" aria-selected={selected}
            onClick={() => onSelect(s.id)} onFocus={() => setFocus(i)}
            className={[
